...
<script>
	...
	function useridCheck(userid){
		...
		if(userid.length<6){
			document.getElementById("idMsg").innerText="아이디를 6자 이상 입력해주세요";
	...
	function pwdLength(my){
		...
		if(my.length>5){
			...
			document.getElementById("pwdMsg1").innerText="비밀번호를 6자 이상 입력해주세요.";
			document.getElementById("pwdMsg1").style.display="block";
	...
	function check(my){
		...
		if(pchk==0){
			...
			if(my.pwd.value.trim()==""){	
    			document.getElementById("pwdMsg1").innerText="비밀번호를 입력하세요";
				document.getElementById("pwdMsg1").style.display="block";
	    		document.getElementById("pwdMsg1").style.color="#e52528";
	    		
	    		if(document.mform.pwd2.value.trim()==""){	
	    			document.getElementById("pwdMsg2").innerText="확인을 위해 새 비밀번호를 다시 입력해주세요.";
					document.getElementById("pwdMsg2").style.display="block";
		    		document.getElementById("pwdMsg2").style.color="#e52528";
	    		}else{
		    		document.getElementById("pwdMsg2").innerText="비밀번호가 불일치합니다.";
				...
				if(my.pwd.value.length<6){
					...
					document.getElementById("pwdMsg1").innerText="사용 불가능한 아이디입니다.";
					document.getElementById("pwdMsg1").style.display="block";
		    		document.getElementById("pwdMsg1").style.color="#e52528";
				}
			}
		...
		if(my.name.value.trim()==""){

			document.getElementById("nameMsg").innerText="이름을 입력하세요";
			document.getElementById("nameMsg").style.display="block";
    		document.getElementById("nameMsg").style.color="#e52528";
    		
    		return false;
    		
		}
		
		if(my.zip_code.value.trim()==""){		
			document.getElementById("addMsg").innerText="주소를 입력하세요";
			document.getElementById("addMsg").style.display="block";
    		document.getElementById("addMsg").style.color="#e52528";
    		
    		if(my.addr_dtl.value.trim()==""){
    			document.getElementById("addMsg").innerText="상세주소를 입력하세요";
    			document.getElementById("addMsg").style.display="block";
        		document.getElementById("addMsg").style.color="#e52528";
    		}
		...
		if(my.email1.value.trim()==""){
			document.getElementById("emailMsg").innerText="이메일을 입력하세요";
			document.getElementById("emailMsg").style.display="block";
    		document.getElementById("emailMsg").style.color="#e52528";
    		
    		if(my.email2.value.trim()==""){
    			document.getElementById("emailMsg").innerText="이메일 도메인을 입력하세요";
    			document.getElementById("emailMsg").style.display="block";
        		document.getElementById("emailMsg").style.color="#e52528";
    		}
		...
		if(my.phone.value.trim()==""){
			document.getElementById("phoneMsg").innerText="전화번호를 입력하세요";
			document.getElementById("phoneMsg").style.display="block";
    		document.getElementById("phoneMsg").style.color="#e52528";
    		return false;
		}
	}
</script>
					...
					<div id="loginWrap">
						<div id="login">
							...
							<form name="mform" method="post" action="memberOk" onsubmit="return check(this)">
						         ...
						         <div id="addrDiv">
						         	<input type="text" id="zip_code" name="zip_code" readonly="readonly" placeholder="우편번호">
									<input type="text" id="addr" name="addr" readonly="readonly" placeholder="주소">
									<input type="text" id="addr_dtl" name="addr_dtl" placeholder="상세주소">
